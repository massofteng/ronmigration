    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($discussion_type == 'housing') {
            ...
            if ($city_id != 0) {
                ...
                $sp_realty_stadt_agglo = "agglo";
                if ($row['sp_realty_stadt_agglo'] == "stadt") {
                    $sp_realty_stadt_agglo = "city";
                }

                $housing_insert_sql = "INSERT INTO housing_details (
                        `market_dis_id`,
                        `looking_to`,
                        `city_or_agglo`,
                        `num_of_rooms`,
                ...
                if ($new_conn->query($housing_insert_sql) === TRUE) {
                    echo $row['user_id'] . ' ' . 'Added</br>';
                } else {
                    //echo "Error: " . $insert_sql . "<br>" . $new_conn->error;
                }
