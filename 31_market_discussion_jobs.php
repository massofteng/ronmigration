...
$sql = "SELECT * FROM ro_advertisement where template_id=0";
        ...
        if ($discussion_type == 'jobs') {
            ...
            if (mysqli_num_rows($check_re) < 1) {
                ...
                if ($city_id != 0) {
                    ...
                    $sp_jobs_employment_type =  $row['sp_jobs_employment_type'] ?? "NULL";
                    $sp_jobs_position =  $row['sp_jobs_position'] ?? "NULL";

                    $de_created = date('Y-m-d H:i:s', $row['created']);

                    $jobs_insert_sql = "INSERT INTO market_jobs_details (
                        `market_dis_id`,
                        `name_of_company`,
                     ...
                     VALUES (
                        ...
                        NULL,
                        0,
                        $sp_jobs_employment_type,
                        $sp_jobs_position,
                        0,
                        '" . $row['sp_jobs_location'] . "',
                        NULL,
                        '" . $de_created . "',
                        '" . $de_created . "'
                        )";
                        //echo $jobs_insert_sql;exit;
                    if ($new_conn->query($jobs_insert_sql) === TRUE) {
                        echo $row['user_id'] . ' ' . 'Added</br>';
                    } else {
                        //echo "Error: " . $insert_sql . "<br>" . $new_conn->error;
                    }
