...
//where profile_id > 121575

$sql = "SELECT * FROM ro_user_profiles where profile_id > 371645 limit 100000";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $user_info = "SELECT id FROM users WHERE active_profile_id=$profile_id ORDER BY id ASC LIMIT 1";
        $user_result = mysqli_query($new_conn, $user_info); //to be optional
        ...
        if ($user_result && mysqli_num_rows($user_result) > 0) {
            ...
            if ($city_result['city_id'] == 'zuerich') {
                ...
                $city_id = 1;
            } else if ($city_result['city_id'] == 'lausanne' || $city_result['city_id'] == 'geneve' || $city_result['city_id'] == 'romandie') {
                $city_id = 3;
